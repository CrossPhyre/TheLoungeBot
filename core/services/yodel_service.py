...
class YodelService:
    def __init__(self, youtube_agent):
        ...
        self._bitrate = 128
        self._allowed_bitrates = [96, 128, 160, 196]
    ...
    def __player_loop__(self):
        while True:
            if self._voice_client is None or self._queue_index == len(self._queue):
                ...
                self._playtime = 0
                self._voice_client.play(discord.FFmpegOpusAudio(y.stream_url, bitrate=self._bitrate, executable=self._ffmpeg_fp, before_options=self._ffmpeg_before_options, options=self._ffmpeg_options))
    ...
    def check_channels(self, text_channel):
        ...
        if self._text_channel is None or self._voice_channel is None:
            message = "I'm not ready for that command yet! I need you to set me up using the '$ydl-init' command."
    ...
    def set_bitrate(self, bitrate):
        if bitrate in self._allowed_bitrates:
            self._bitrate = bitrate

            success = True
            message = f"Set the bitrate to {bitrate}kbps, boss! The new bitrate will go into effect with the next song!"
        else:
            success = False
            message = f"Sorry, boss, but I can't use that bitrate. I only know how to use one of the following values: {', '.join(self._allowed_bitrates)}."
